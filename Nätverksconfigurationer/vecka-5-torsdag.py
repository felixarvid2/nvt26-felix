...
!kopplingen till borås
interface GigabitEthernet0/0/1
 description Länk mot Borås
...
! Undanta IP-adresser (Gateway och reserverade adresser)
ip dhcp excluded-address 192.168.2.1 192.168.2.09
ip dhcp excluded-address 192.168.2.65 192.168.2.74
ip dhcp excluded-address 192.168.2.193 192.168.2.202

! DHCP-pool för LAGER (VLAN 40)
ip dhcp pool LAGER
...
! DHCP-pool för GAST (VLAN 50)
ip dhcp pool GAST_BORAS
 network 192.168.2.64 255.255.255.192
 default-router 192.168.2.65
 dns-server 8.8.8.8
 exit

! DHCP-pool för DRIFT (VLAN 99)
ip dhcp pool DRIFT_BORAS
...
! Subinterface för VLAN 40, 50, 99
...
! Trunk mot R-Boras-1
interface GigabitEthernet 0/1
 description Trunk mot Router Boras
 switchport trunk encapsulation dot1q
 switchport mode trunk
 switchport trunk native vlan 999
